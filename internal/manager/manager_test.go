...
func TestEnsureRoot_DefaultOrTrue(t *testing.T) {
    ...
    t.Run("require=nil default false", func(t *testing.T) {
        ...
        if err != nil { t.Fatalf("expected nil error when require_root defaults to false, got %v", err) }
    })
    ...
    t.Run("require=true", func(t *testing.T) {
        ...
        err := ensureRoot("pkg", "step", req)
        if euid == 0 {
            if err != nil { t.Fatalf("expected nil error for root, got %v", err) }
        } else {
            if err == nil { t.Fatalf("expected error for non-root when require_root=true") }
        }
    })
}
...
func TestCmpVersion(t *testing.T) {
    ...
    sign := func(x int) int { if x > 0 { return 1 } else if x < 0 { return -1 } ; return 0 }
    for _, c := range cases {
